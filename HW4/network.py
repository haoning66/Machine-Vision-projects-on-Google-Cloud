...
class LSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.f_t = nn.Linear(input_size, hidden_size, bias=True)
        self.i_t = nn.Linear(input_size, hidden_size, bias=True)
        self.C_tt = nn.Linear(input_size, hidden_size, bias=True)
        self.o_t = nn.Linear(input_size, hidden_size, bias=True)
    ...
    def forward(self, x, hidden):
        # TODO:
        ht_1, ct_1 = hidden
        c_t = self.sigmoid(self.f_t(x))*ct_1+self.sigmoid(self.i_t(x))*self.tanh(self.C_tt(x))
        h_t = self.sigmoid(self.o_t(x))*self.tanh(c_t)
        hidden = (h_t,c_t)
